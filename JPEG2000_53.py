...
SIZE_X = int(PXL_NUM)
SIZE_Y = int(PXL_NUM)
LPFpadded = np.zeros(shape = [PXL_NUM + 4, PXL_NUM + 4])
HPFpadded = np.zeros(shape = [PXL_NUM + 4, PXL_NUM + 4])
LPFdata = np.zeros(shape = [PXL_NUM, PXL_NUM])
HPFdata = np.zeros(shape = [PXL_NUM, PXL_NUM])

ILPFpadded = np.zeros(shape = [PXL_NUM + 4, PXL_NUM + 4])
IHPFpadded = np.zeros(shape = [PXL_NUM + 4, PXL_NUM + 4])
...
def DWT(SIZE_X, SIZE_Y, LPFpadded, HPFpadded, LPFdata, HPFdata, DWTdata, cycle):
    while cycle > 0:
        f53.Padding(SIZE_X, SIZE_Y, LPFpadded, DWTdata)
        f53.Padding(SIZE_X, SIZE_Y, HPFpadded, DWTdata)
        f53.getL(SIZE_X, SIZE_Y, LPFdata, LPFpadded)
        f53.getH(SIZE_X, SIZE_Y, HPFdata, HPFpadded)
        SIZE_X = int(SIZE_X / 2)
        
        f53.Padding(SIZE_X, SIZE_Y, LPFpadded, LPFdata)
        f53.Padding(SIZE_X, SIZE_Y, HPFpadded, LPFdata)
        f53.getLLLH(SIZE_X, SIZE_Y, DWTdata, LPFdata, LPFpadded, HPFpadded)
        f53.Padding(SIZE_X, SIZE_Y, LPFpadded, HPFdata)
        f53.Padding(SIZE_X, SIZE_Y, HPFpadded, HPFdata)
        f53.getHLHH(SIZE_X, SIZE_Y, DWTdata, HPFdata, LPFpadded, HPFpadded)
        SIZE_Y = int(SIZE_Y / 2)
        cycle -= 1
...
def IDWT(SIZE_X, SIZE_Y, IHPFpadded, ILPFpadded, ILPFdataL, ILPFdataH, IHPFdataL, IHPFdataH, DWTdata):
    ...
    while cnt < num:
        f53.LVL1Interp(SIZE_X, SIZE_Y, ILPFdataL, ILPFdataH, IHPFdataL, IHPFdataH, DWTdata)
        SIZE_Y *= 2
        f53.Padding(SIZE_X, SIZE_Y, IHPFpadded, IHPFdataL)    
        f53.Padding(SIZE_X, SIZE_Y, ILPFpadded, ILPFdataL)
        f53.I_getLH(SIZE_X, SIZE_Y, ILPFdata, IHPFpadded, ILPFpadded)             
        f53.Padding(SIZE_X, SIZE_Y, IHPFpadded, IHPFdataH)
        f53.Padding(SIZE_X, SIZE_Y, ILPFpadded, ILPFdataH)
        f53.I_getLH(SIZE_X, SIZE_Y, IHPFdata, IHPFpadded, ILPFpadded)
        f53.LVL2Interp(SIZE_X, SIZE_Y, ILdata, ILPFdata, IHdata, IHPFdata)
        SIZE_X *= 2
        f53.Padding(SIZE_X, SIZE_Y, IHPFpadded, IHdata)
        f53.Padding(SIZE_X, SIZE_Y, ILPFpadded, ILdata)
        f53.I_getOrig(SIZE_X, SIZE_Y, DWTdata, IHPFpadded, ILPFpadded)
        cnt += 1
